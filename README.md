...
- **Guests.** Someone who opens an invite link without signing in joins as a guest and types their own name; the default user's name is only pre-filled for the host (the Start link), a signed-in user, or the dashboard's own Join controls. The server still counts that request as the default user, so the guest's attendance row is linked to the default account, though their chosen name is shown in the meeting, chat and history. Telling a guest apart from the default user would need sign-in to be mandatory.
